# slightly modfified from Yuan Gao's Riskaware scenario analysis
...
def extract_important_information(json_file_path):
    ...
    for lanelet in data.get("lanelet", []):
        ...
        # Handle adjacentLeft
        adjacent_right = lanelet.get("adjacentLeft")
        if isinstance(adjacent_right, dict):
            lanelet_info["adjacentLeft"] = [lanelet.get("adjacentLeft").get("ref", "None")]
        elif isinstance(adjacent_right, list):
            lanelet_info["adjacentLeft"] = [
                ref.get("ref", "None") for ref in adjacent_right if isinstance(ref, dict)
            ]

        # Handle adjacentRight
        adjacent_right = lanelet.get("adjacentRight")
        if isinstance(adjacent_right, dict):
            lanelet_info["adjacentRight"] = [lanelet.get("adjacentRight").get("ref", "None")]
        elif isinstance(adjacent_right, list):
            lanelet_info["adjacentRight"] = [
                ref.get("ref", "None") for ref in adjacent_right if isinstance(ref, dict)
            ]
        
        # Handle successor
        successor = lanelet.get("successor")
        if isinstance(successor, dict):
            lanelet_info["successor"] = [successor.get("ref", "None")]
        elif isinstance(successor, list):
            lanelet_info["successor"] = [
                ref.get("ref", "None") for ref in successor if isinstance(ref, dict)
            ]

        # Handle predecessor
        predecessor = lanelet.get("predecessor")
        if isinstance(predecessor, dict):
            lanelet_info["predecessor"] = [predecessor.get("ref", "None")]
        elif isinstance(predecessor, list):
            lanelet_info["predecessor"] = [
                ref.get("ref", "None") for ref in predecessor if isinstance(ref, dict)
            ]
